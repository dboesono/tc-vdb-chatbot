...
# how many user⇄assistant exchanges to keep
MAX_TURNS = 5
...
def build_vectordb_from_emails(emails):
    ...
    for i, email in enumerate(emails):
        ...
        # ─── NEW: token‐count & conditional summarization ───
        tokens = tokenizer(combined_text, return_attention_mask=False)["input_ids"]
        if len(tokens) > 8192:
            logging.info(f"Email {i} is {len(tokens)} tokens; running DeepSeek summarization…")
            combined_text = summarize_email_with_deepseek(combined_text, max_words=300)

            # re‐count and warn if still too big
            if len(tokenizer(combined_text)["input_ids"]) > 8192:
                logging.warning(f"Summary still >8192 tokens; will chunk summary.")
...
def summarize_email_with_deepseek(long_text: str, max_words: int = 300) -> str:
    """
    Use your DeepSeek conversational endpoint to produce a ~max_words summary
    that preserves sender, date, subject, key points, and action items.
    """
    system = "You are an email assistant. Summarize emails preserving sender, date, subject, main points, and action items."
    user = (
        f"Please summarize the following email into no more than {max_words} words:\n\n"
        f"{long_text}"
    )
...
if user_input:
    ...
    if st.session_state.all_emails is None:
        ...
        try:
            ...
            if e.code == 15301:
                ...
                relevant_emails = retrieve_top_k_emails(user_input, k=35)
                emails_for_analysis = relevant_emails
            else:
                raise
    ...
    with st.chat_message("assistant"):
        ...
        # right before you do re.split(...)
        if isinstance(ai_reply, tuple):
            ai_reply = ai_reply[0]    # grab just the text
